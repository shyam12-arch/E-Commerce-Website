...
@login_required
def orders(request):
 ops = OrderPlaced.objects.filter(user=request.user)
 return render(request, 'app/orders.html',{'order_placed':ops})
...
def laptop(request,data=None):
 if data==None:
  ...
  laptops = Product.objects.filter(category='L').filter(discounted_price__lte=80000)
 elif data=='above':
  laptops = Product.objects.filter(category='L').filter(discounted_price__gte=80000)
...
def topwear(request,data=None):
 if data==None:
  ...
  topwears = Product.objects.filter(category='TW').filter(discounted_price__lte=300)
 elif data=='above':
  topwears = Product.objects.filter(category='TW').filter(discounted_price__gte=300)
 return render(request, 'app/top_wear.html',{'topwears':topwears})

def bottomwear(request,data=None):
 if data==None:
  bottomwears = Product.objects.filter(category='BW')
 elif data=='below':
  bottomwears = Product.objects.filter(category='BW').filter(discounted_price__lte=300)
 elif data=='above':
  bottomwears = Product.objects.filter(category='BW').filter(discounted_price__gte=300)
 return render(request, 'app/bottom_wear.html',{'bottomwears':bottomwears})
